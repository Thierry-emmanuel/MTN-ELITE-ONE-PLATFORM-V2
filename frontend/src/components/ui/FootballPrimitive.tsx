...
export const PageHero = memo(({
  eyebrow, title, subtitle, accentColor = 'green', badge,
}: PageHeroProps) => {
...
export const ScoreDisplay = memo(({ home, away, status, isLive, size = 'sm' }: ScoreDisplayProps) => {
  const finished = status === 'FT';
  const hasScore = home !== null && away !== null;
  const textSize = size === 'lg' ? 'text-3xl lg:text-4xl' : 'text-xl';

  if (!hasScore) {
    return (
      <div className="flex items-center gap-1.5 text-muted-foreground/40">
        <span className={`font-display ${textSize}`}>vs</span>
      </div>
    );
  }

  return (
    <div className={`flex items-center gap-1 font-display ${textSize} tabular-nums`}>
      <span className={finished ? 'text-foreground' : isLive ? 'text-[#CE1126]' : 'text-foreground'}>
        {home}
      </span>
...
// ─── Match Meta (venue / referee / attendance) ────────────────────────────────
...
export const SummaryCard = memo(({ label, value, sub, color = 'text-accent', badge, delay = 0 }: SummaryCardProps) => (
  <motion.div
    ...
    className="bg-surface-elevated/50 border border-border/50 rounded-xl px-4 py-3.5 text-center space-y-0.5"
  >
