          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="h-48 rounded-3xl bg-white/[0.02] border border-border/40 animate-pulse"
                />
              ))}
            </div>
                ...
                >
                  ...
                  <div className="p-6 flex flex-col justify-between h-full space-y-6">
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                          ...
                          <span>{club.city === 'Garoua' ? 'Stade Roumdé Adjia' : club.city === 'Yaoundé' ? 'Stade Ahmadou Ahidjo' : 'Stade Municipal'}</span>
                        </p>
                      </div>
                      <div className="shrink-0 h-16 w-16 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center justify-center p-2.5 shadow-inner group-hover:scale-105 transition-transform">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-border/30">
                      <div className="flex items-center gap-4">
                        <div className="text-center">
                          <div className="text-xs font-mono font-bold text-white/80">
                            {club.id === 'cot' ? '18' : club.id === 'cnk' ? '18' : '18'}
                          </div>
                      ...
                      <Link
                        to={`/clubs/${club.id}`}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 hover:bg-accent hover:text-black border border-white/10 hover:border-transparent text-xs font-bold transition-all duration-200"
                      >
